    ...
    healthLoss(damage) {
        ...
        if (damage < this.health) {
            if (this.nbDmgBeforeShield <= 0) {
                var char = new Char("mini", this.shape.position.x + 2, this.shape.position.y, 0, 1, 1000, 45, 2, 1, 3, 3);
                charsAI.push(char);
                char.setStrategy(new guaranteedAI(char))
                chars.push(char);
                char.applyStrategy()

                var char2 = new Char("mini", this.shape.position.x - 2, this.shape.position.y, 0, 1, 1000, 45, 2, 1, 3, 3);
                charsAI.push(char2);
